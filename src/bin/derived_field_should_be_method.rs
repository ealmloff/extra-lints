...
fn run() -> Result<()> {
    ...
    let artifact_dir = target_dir.join("derived_field_should_be_method");
    let collect_target_dir = target_dir.join("derived_field_should_be_method_collect");
    let emit_target_dir = target_dir.join("derived_field_should_be_method_emit");
